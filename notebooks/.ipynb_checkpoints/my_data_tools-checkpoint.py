...
def get_df(*args):
    df = args[0] # the particular dataframe
    quarter = args[1] # quarter and the specific year
    columns = args[2] # chosen columns
    decision = args[3] # chosen columns
    var_name = args[4] # name of the category
    value_name = args[5] # name of the values

    if decision == 'selective':
        df = df.loc[df['Quarter']==quarter, columns]
    elif decision == 'range':
        df = df.loc[df['Quarter']==quarter, columns[0]:columns[1]]
    # Reshaping the data
